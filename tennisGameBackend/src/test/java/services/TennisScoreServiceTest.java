...
class TennisScoreServiceTest {
    ...
    void simpleStraightWinForA() {
        ...
        assertThat(s.state).isEqualTo(TennisScoreService.GameState.WIN_A);
    }
    
    @Test
    void simpleStraightWinForB() {
        TennisScoreService.TennisScore s = service.process("BBBB");
        assertThat(s.state).isEqualTo(TennisScoreService.GameState.WIN_B);
    }
    
    @Test
    void deuceScenario() {
        TennisScoreService.TennisScore s = service.process("ABABAB");
        assertThat(s.state).isEqualTo(TennisScoreService.GameState.DEUCE);
    }
    
    @Test
    void advantageForA() {
        TennisScoreService.TennisScore s = service.process("ABABABA");
        assertThat(s.state).isEqualTo(TennisScoreService.GameState.ADV_A);
    }
    ...
    void scoreDescriptions() {
        ...
        score.pointsA = 1;
        score.pointsB = 2;
        score.state = TennisScoreService.GameState.ONGOING;
        assertThat(service.getScoreString(score)).isEqualTo("Player A : 15 / Player B : 30");
        
        score.state = TennisScoreService.GameState.WIN_A;
